        ...
        with open('input.csv') as csvfile_input:
            ...
            for row_input in reader_input:
                ...
                if host_input:
                    ...
                    print('Processing fact file ' + fact_file )
                    msg = ""
                    if os.path.isfile(fact_file):
                        ...
                        with open(fact_file) as csvfile_fact:
                            ...
                            for row_fact in reader_factfile:
                                ...
                                output_dict = {'Hostname input': host_input, 'Hostname output': host_fact, 'IP input': ipaddress_input, 'IP fact': ipaddress_fact, 'CPU core input': cpucore_input, 'CPU core fact': cpucore_fact, 'RAM input': ram_input, 'RAM fact': ram_fact, 'Disk input': disk_input, 'Disk fact': disk_fact, 'Message': msg}
                                output_list.append(output_dict)
                                fact_row_ctr += 1
                            print('Finished processing fact file ' + fact_file)
                    else:
